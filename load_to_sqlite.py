...
def load_to_occurrences(csvfile):
    ...
    cur = con.cursor()
    # default is that first line are the headers
    with open(csvfile, 'rt', encoding='utf-8') as f:
        # comma is default delimiter,changing it to \t
        ...
        to_db = [
            (i['gbifID'],
             i['datasetName'],
                ...
                i['decimalLongitude'],
                i['taxonKey'],
                i['verbatimScientificName']) for i in dr]
    ...
    with open(csvfile, 'rt', encoding='utf-8') as f:
        ...
        to_db = [
            (i['gbifID'],
             i['identifier'],
                i['license'],
                i['rightsHolder']) for i in dr]

    cur.executemany(
        "INSERT OR IGNORE INTO occurrence_images (gbifid, image_url, license, rights_holder) VALUES (?, ?, ?, ?);",
        to_db)
    con.commit()
    con.close()
...
def load_to_wiki_info(csvfile):
    ...
    # default is that first line are the headers
    with open(csvfile, 'rt', encoding='utf-8') as f:
        # comma is default delimiter,changing it to \001
        dr = csv.DictReader(f, delimiter='\001')
...
def main():
    ...
    try:
        ...
        logger.exception(
            'Unable to load to occurrence_images table in database')
    try:
