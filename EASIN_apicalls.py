...
def api_call_by_kingdom(kingdom, f):
    ...
    skip_records = 0
    url = 'https://easin.jrc.ec.europa.eu/api/cat/kingdom/'+kingdom+'/take/' + str(pagination_max) + '/skip/' + str(skip_records)
    r = requests.get(url)
    header_length = 1000
    tot_length = 0
    while (header_length >= 1000):
        ...
        skip_records += 50
        url = 'https://easin.jrc.ec.europa.eu/api/cat/kingdom/animalia/take/' + str(pagination_max) + '/skip/' + str(skip_records)
        r = requests.get(url)
